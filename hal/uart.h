...
typedef struct {
  USART_TypeDef
      *Instance; // Pointer to the hardware base address, this speeds up a lot
  UART_Config_t Init;  // config settings init
  uint8_t *pTxBuff;    // Pointer to TX buffer
  uint16_t txTranSize; // total transfer size -> 16 bit because of DMA down and
                       // up counter is 16 bit

...
void GPIO_Init(GPIO_TypeDef *GPIOx, GPIO_PinConfig_t *Config) { //Function initiator
  // Enables GPIO clocks
  if (GPIOx == GPIOA) {
  ...
  // Set Output Speed
  GPIOx->OSPEEDR &= ~(3UL << (pin * 2)); //undo or invert
...
void GPIO_DeInit(GPIO_TypeDef *GPIOx, GPIO_PinConfig_t *Config) { //Function initiator
  ...
  // Reset Push Pull -> usually none
  GPIOx->PUPDR &= ~(3UL << (pin * 2));
  
  // Resetting Alternate Function
  if (Config->Mode == GPIO_MODE_AF) {
    // To find if it is high or low register
    uint8_t afr_index =
        pin >> 3; // This is really cool! Divides by 8. 1000 (8) -> 0001 (1)!
